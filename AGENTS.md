...
- **Entry**: `MainActivity.kt` (`@AndroidEntryPoint`, splash, edge-to-edge) → `JournalTheme` → `ui/main/MainScreen.kt`: 5-tab NavigationBar (Journal / Statistics / Substances / SaferUse / Settings) + a `NavDisplay`; content gated behind a "conditions accepted" DataStore flag.
- **Navigation**: Navigation 3. Type-safe `@Serializable` route keys — every one implementing `NavKey` — live in `navigation/routes/` (one file per tab plus `TabRoutes.kt` for the bottom-bar tabs). `navigation/Nav3TabManager.kt` owns the five per-tab `NavBackStack`s (`rememberNavBackStack`, so the selected tab, every stack and every entry's saved state survive configuration changes and process death) and knows which tab a route key belongs to; `navigation/Nav3DestinationProvider.kt` registers each key once with `entry<T>` and `MainScreen` renders them through `NavDisplay` with `Nav3TabManager.decoratedEntries()`, which keeps the saved state and `ViewModelStore` of the tabs that are not currently on screen (the `saveState`/`restoreState` behavior of the old nested graphs). A destination whose ViewModel needs its navigation argument receives the key through a Hilt assisted factory (`@AssistedInject` + `@AssistedFactory`, wired as `hiltViewModel<VM, VM.Factory>(creationCallback = { it.create(route) })`) and the screen takes that `viewModel` as a required parameter; there is no `NavController` and no `SavedStateHandle.toRoute`. Keys that are not part of the statistics, substances, safer-use or settings set are journal-tab routes. The shared transition in `navigation/composableWithTransitions.kt` is deliberately uniform: one 90 ms cross-fade with no directional movement, so pushing, popping, switching tabs and the predictive-back gesture preview all look the same. The `addIngestion` and `addCustomUnit` flows are ordinary screen sequences rooted at `AddIngestionRoute`/`AddCustomUnitsRoute` and ended with `dismissFlow`; `journal://open/...` deep links are matched in `MainScreen`.
- **Screens/ViewModels**: one `XScreen.kt` + `XViewModel.kt` pair per feature; screens receive the VM as `viewModel: XViewModel = hiltViewModel()` — except when the VM needs its navigation argument, in which case the entry provider passes it and the parameter is required. VMs combine repository Flows into `StateFlow` via `stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000))`.
- **Data layers**:
  ...
  - Localization: `localization/I18n.kt` loads `assets/lang/*.json` (keys like `en_us`, `zh_cn`, `zh_tw`; `supported.json` lists them), en_us fallback + localized overlay, `{placeholder}` replacement; `i18n()`/`i18nOrDefault()` composables; deferred translation via `I18nText(key, params)`. Missing keys render `"missing_key"`. `I18n.translate` is on the composition hot path, so it reads a `@Volatile` immutable snapshot and only the (re)load path takes a lock — never hold that lock while doing asset I/O.
  ...
  - Desktop widgets: `ui/widgets/` — `StatsWidgetProvider` owns rendering, `StatsWidgetSync` owns the refresh chain (journal writes via `JournalDataEvents`, app language, `I18n.stringsChanged`, app foregrounding, the system update broadcast). All triggers conflate into one off-main worker: `RemoteViews`/`updateAppWidget`/`I18n`/`SubstanceRepository` must never run on the main thread, and nothing is queried while no widget is placed.
...
| `ui/main/MainScreen.kt` | Tab scaffold, `NavDisplay`, bottom-bar wiring, notification/deep-link intent dispatch, conditions gate, language push to I18n |
| `ui/main/navigation/*` | `routes/` type-safe `NavKey` route classes; `Nav3TabManager.kt` (per-tab back stacks, selected tab, tab of a key) + `Nav3DestinationProvider.kt` (key→content registration) |
...
- **Coverage**: JVM unit tests (`TestRegex.kt`, `TestParse.kt`, `TestDates.kt`, `TestNav3TabManager.kt`, `DoubleReadableExtensionKtTest.kt`, plus data-layer tests) + 1 instrumented smoke test (`ExampleInstrumentedTest.kt`). No Robolectric, no Compose UI tests, no test resources — test data is inline triple-quoted strings. Do not add tests that read `assets/` (impossible without Robolectric).
