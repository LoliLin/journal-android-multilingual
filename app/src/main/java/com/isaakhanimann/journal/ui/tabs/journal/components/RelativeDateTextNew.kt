...
@Composable
fun RelativeDateTextNew(dateTime: Instant, style: TextStyle = MaterialTheme.typography.bodyMedium) {
    ...
    val relativeTime = when {
        duration.toDays() > 500 -> "${roundToOneDecimal(
            duration.toDays().toDouble() / 365.0
        )} years ago"
        duration.toDays() > 60 -> "${roundToOneDecimal(
            duration.toDays().toDouble() / 30.0
        )} months ago"
        duration.toDays() > 20 -> "${roundToOneDecimal(
            duration.toDays().toDouble() / 7.0
        )} weeks ago"
        duration.toHours() > 24 -> "${roundToOneDecimal(
            duration.toHours().toDouble() / 24.0
        )} days ago"
        duration.toHours() > 0 -> "${roundToOneDecimal(
...
fun roundToOneDecimal(num: Double): String = try {
